 ...
 * there. The `home_keeper.snooze_task` / `skip_task` / `pull_forward_task`
...
export function skipSnoozeSchema(): FormField[] {
  return [
    ...
    { name: 'allow_pull_forward', selector: selBool() },
  ];
}
...
export function skipSnoozeFlags(options: {
  ...
  allow_skip?: unknown;
  allow_pull_forward?: unknown;
}): { allowSnooze: boolean; allowSkip: boolean; allowPullForward: boolean } {
  return {
    allowSnooze: boolOr(options?.allow_snooze, true),
    allowSkip: boolOr(options?.allow_skip, true),
    allowPullForward: boolOr(options?.allow_pull_forward, true),
  };
}
