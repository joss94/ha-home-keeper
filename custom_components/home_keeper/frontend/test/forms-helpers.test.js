...
describe('skipSnoozeSchema', () => {
  it('renders exactly the three switches, in the order the section reads', () => {
    expect(skipSnoozeSchema()).toEqual([
      ...
      { name: 'allow_pull_forward', selector: { boolean: {} } },
    ]);
  });
});
...
describe('skipSnoozeFlags', () => {
  it('reads all three switches when they are stored', () => {
    expect(
      skipSnoozeFlags({ allow_snooze: true, allow_skip: false, allow_pull_forward: true }),
    ).toEqual({
      allowSnooze: true,
      allowSkip: false,
      allowPullForward: true,
    });
    expect(
      skipSnoozeFlags({ allow_snooze: false, allow_skip: true, allow_pull_forward: false }),
    ).toEqual({
      allowSnooze: false,
      allowSkip: true,
  ...
  it('treats each switch independently', () => {
    ...
    expect(skipSnoozeFlags({ allow_pull_forward: false })).toEqual({
      allowSnooze: true,
      allowSkip: true,
      allowPullForward: false,
    });
  ...
  it('reads any other falsy value as off', () => {
    ...
    expect(
      skipSnoozeFlags({ allow_snooze: 0, allow_skip: '', allow_pull_forward: 0 }),
    ).toEqual({
      allowSnooze: false,
      allowSkip: false,
      allowPullForward: false,
