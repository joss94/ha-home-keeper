...
def async_register(hass: HomeAssistant) -> None:
    ...
    websocket_api.async_register_command(hass, ws_pull_forward_task)
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "home_keeper/pull_forward_task",
        vol.Required("task_id"): str,
    }
)
@websocket_api.async_response
async def ws_pull_forward_task(
    hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]
) -> None:
    coord = _coordinator(hass)
    if coord is None:
        ...
        return
    try:
        task = await coord.store.pull_forward_task(msg["task_id"])
    except KeyError:
        _err(
            hass, connection, msg, "not_found", "task_not_found", task_id=msg["task_id"]
        )
