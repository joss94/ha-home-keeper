...
def test_pull_forward_event(ha, ha_token):
    """pull_forward_task fires its event and moves next_due to now without
    ...
    def setup_task():
        ...
        # next_due lands a full interval in the future — the case pull-forward is
    ...
    pulled = _by_type(
        _run(ha_token, ["home_keeper_task_pulled_forward"], pull_forward, expected=1)
    )
    assert "home_keeper_task_pulled_forward" in pulled
    payload = pulled["home_keeper_task_pulled_forward"][0]
    assert payload["task_id"] == created_id["id"]
    ...
    # were before the pull-forward — only next_due moved.
